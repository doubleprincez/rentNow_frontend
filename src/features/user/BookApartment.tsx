...
const BookApartment = ({bookingId}: BookApartmentI) => {
    ...
    const fetchRent = async () => {
        if (isLoading) return;
        setIsLoading(true);
        await AxiosApi('user', token).get(baseURL + '/rented-apartment/' + bookingId)
...

                {
                    rent && <div className="space-y-3">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span
                                className="text-gray-600">Cost: <strong> {rent?.currency_code}{formatAmountNumber(rent?.amount)}</strong></span>
                        </div>
                        <div className="flex items-center gap-2">
                            <Calendar1Icon className="text-orange-500"/>
                            <span
                                className="text-gray-600">Duration: <strong>{simpleDateFormat(rent?.start)} - {simpleDateFormat(rent?.end)}</strong></span>
                        </div>
                        <div className="flex items-center gap-2">
                        ...
                        <div className="font-semibold text-xs text-red-500 text-justify">
                            Please contact the agent directly through the agent details provided and get the account to make
                            payment to. then upload your proof of payment here for verification and documentation.
                        </div>
                        <div className="flex space-x-2">
                            To View Rent History: <Link href={"/user/rent"}
                                                        className="font-bold flex space-x-2"><LinkIcon/> Click Here</Link>
                        </div>
                    </div>
                                ...
                                <div className="flex flex-col gap-4">
...

                                    {
                                        ...
                                        <div className="flex items-center gap-2">
                                            <CreditCardIcon className="text-orange-500"/>
                                            <span
                                                className="text-gray-600">Security Deposit: {formatAmountNumber(apartment.security_deposit) || 'Not specified'}</span>
                                        </div>
                                    }
                                    {
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span
                                                className="text-gray-600">Business: {apartment.business_name || 'Not specified'}</span>
                                        </div>
                                    }
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span
                                            className="text-gray-600">Security Deposit: {apartment.security_deposit || 'None'}</span>
                                    </div>
                                        ...
                                        apartment?.agent_type == 'agent' ? <>
...

                                            {
                                                apartment?.business_address && <div className="flex items-center gap-2">
                                                    ...
                                                    <span
                                                        className="text-gray-600">Phone: {apartment?.business_address}</span>
                                                </div>
                                            }

                                            {
                                                apartment?.business_email && <div className="flex items-center gap-2">
                                                    ...
                                                    <span
                                                        className="text-gray-600">Phone: {apartment?.business_email}</span>
                                                </div>
                                            }
                                            {
                                                apartment?.business_phone && <div className="flex items-center gap-2">
                                                    <PhoneCallIcon className="text-orange-500"/>
                                                    <span
                                                        className="text-gray-600">Phone: {apartment?.business_phone}</span>
                                                </div>
                                            }
                                        </>
                                    }
