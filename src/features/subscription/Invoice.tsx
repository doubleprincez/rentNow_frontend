import {bankInterface, PaystackInterface, TransactionHistory} from "@/types/subscription";
import React, {useRef} from "react";
import {useReactToPrint} from "react-to-print";
import {FacebookIcon, FacebookShareButton, WhatsappIcon, WhatsappShareButton} from "react-share";
import {extractClassName, formatAmountNumber, setColor, simpleDateFormat} from "@/lib/utils";
...
const Invoice = ({transaction}: InoviceI) => {
    ...
    return (
        <div className={"flex justify-center pt-2 items-start bg-gray-100 min-h-[600px]"}>
            <div className="p-2 w-3/4 md-w-4/6 max-w-lg mx-auto bg-white shadow-lg rounded-lg ">
        ...
        return <>
            ...
            <div>
                ...
                <div className={"text-end "}>
                    Account Name: <div
                    className="text-mono word-break font-semibold"> {String(bank?.account_name)}</div>
                </div>
                <div className={" text-end "}>
                    Account Number: <div className="text-mono  word-break font-semibold">{bank?.account_number}</div>
                </div>
            </div>
            <div className={""}>User Details</div>
            <div className={"text-end pr-3"}>
                <div className={" placeholder:pr-3"}>Account Name: <div
                    className=" word-break text-mono font-semibold">{transaction.payable?.meta?.name}</div></div>
                <div className={" pr-3"}>Email: <div
                    className="text-mono font-semibold word-break">{transaction.payable?.meta?.email}</div></div>
            </div>
        ...
        return (
            <>
                ...
                <div>
                    ...
                    {/*<div className={"text-end word-break "}>Bank Name: {String(bank?.bank_name)} </div>*/}

                    <div className={"text-end "}>
                        Payment Reference:
                        <div className="text-mono word-break font-semibold"> {(paystack?.reference)}</div>
                    </div>
                    <div className={" text-end "}>
                        {/*Account Number:*/}
                        {/*// <div className="text-mono  word-break font-semibold">{bank?.account_number}</div>*/}
                    </div>
                </div>
                <div className={""}>User Details</div>
                <div className={"text-end pr-3"}>
                    <div className={" placeholder:pr-3"}>Account Name: <div
                        className=" word-break text-mono font-semibold">{paystack?.meta.name}</div></div>
                    <div className={" pr-3"}>Email: <div
                        className="text-mono font-semibold word-break">{paystack?.meta?.email}</div></div>
                </div>
    ...
    return <div ref={ref}>
        <div className="p-4 space-x-8 space-y-2 border rounded min-h-[500px]">
            ...
            <div className={"mt-1 grid grid-cols-2 gap-8"}>
                {
                    transaction?.payable?.bank ? withdrawal(transaction?.payable?.bank) : paystack(transaction?.payable)
                }
            </div>
        </div>
        <hr/>
