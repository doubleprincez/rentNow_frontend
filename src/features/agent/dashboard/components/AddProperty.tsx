...
const AddProperty: React.FC = () => {
  ...
  const {
    ...
    defaultValues: {
      ...
      currency_code: 'NGN',
      country_code: 'NG',
      images: Array(MAX_FILES).fill(null),
      videos: Array(MAX_FILES).fill(null)
    }
  });
  ...
  const onSubmit = async (data: PropertyFormData) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setValue('state_code', '');
        setValue('city_code', '');
        setValue('images', Array(MAX_FILES).fill(null));
        setValue('videos', Array(MAX_FILES).fill(null));
                ...
                {Array(MAX_FILES).fill(null).map((_, index) => (
                  <div key={`image-${index}`} className="relative group">
                    ...
                    <div className="w-full aspect-square border-2 border-gray-300 border-dashed flex items-center justify-center rounded-lg bg-gray-100 group-hover:border-orange-500 transition-all relative">
                      {imagePreview[index] ? (
                        <>
                          <img src={imagePreview[index]} alt={`Preview ${index + 1}`} className="w-full h-full object-cover rounded-lg" />
                          <button
                            type="button"
                ...
                {Array(MAX_FILES).fill(null).map((_, index) => (
                  <div key={`video-${index}`} className="relative group">
                    ...
                    <div className="w-full aspect-square border-2 border-gray-300 border-dashed flex items-center justify-center rounded-lg bg-gray-100 group-hover:border-orange-500 transition-all relative">
                      {videoPreview[index] ? (
                        <>
                          <video src={videoPreview[index]} className="w-full h-full object-cover rounded-lg" />
                          <button
                            type="button"
