...
const FindHomes = () => {
    ...
    const [apartments, setApartments] = useState<Apartment[]>([]); // Explicitly type as Apartment array
    ...
    //         try {
    //             const response = await fetch('https://api.rent9ja.com.ng/api/apartments');
    //             if (!response.ok) {
    ...
    //                 throw new Error('Invalid data format: expected an array of apartments');
    //             }
        ...
        return (
            <div className="w-full flex justify-center items-center h-screen">
                <div className="text-red-500 text-center">
                    <p className="font-semibold">Error loading apartments</p>
                    ...
                    <div className="w-[800px] py-4 mx-auto flex gap-2 sm:gap-4 overflow-x-auto no-scrollbar md:justify-center">
                        {tabslist.map((tab, index) => (
                            <TabsTrigger 
                                key={index} 
                                value={tab.value} 
                                className='p-2 flex gap-2 shadow-md hover:bg-orange-50 data-[state=active]:bg-orange-100'
                            >
                    ...
                    <TabsContent key={tab.value} value={tab.value}>
                        <div className='w-full grid grid-cols-1 sml:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4'>
                            {Array.isArray(apartments) && apartments
                                .filter((apt) => tab.value === 'all' || apt.type === tab.value)
                                .map((apt, index) => (
                                    <div 
                                        key={apt.id || index} 
                                        className='flex flex-col gap-4 p-2 md:p-4 bg-white shadow-md rounded-2xl hover:shadow-lg transition-shadow'
                                    >
                                        <div className='flex flex-col gap-2'>
                                            ...
                                            <div className='flex flex-col gap-2'>
                                                ...
                                                <div className='flex items-center gap-2'>
                                                    <Banknote className='text-orange-500 h-4 w-4'/>
