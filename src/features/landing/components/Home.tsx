                    ...
                    <div className="relative z-10 flex items-center w-full h-full bg-black bg-opacity-40">
                        <div className="w-full flex flex-col gap-6 px-2 md:px-6">
                            ...
                            >
                                ...
                                >
                                    <SelectTrigger className="w-full h-8 md:h-12 text-[.8em] md:text-[.9em]">
...
//     // Fetching options on component mount - this is for the form
//     useEffect(() => {
//         const fetchOptions = async () => {
//             try {
//                 const response = await fetch('/api/apartments');
...
//                     const uniqueCategories = [...new Set(data.data.data.map(apt => apt.category))].filter(Boolean);
//                     const uniqueStates = [...new Set(data.data.data.map(apt => apt.state_code))].filter(Boolean);
//                     const uniqueCountries = [...new Set(data.data.data.map(apt => apt.country_code))].filter(Boolean);
...
//             if (searchParams.category !== 'all') queryParams.append('category', searchParams.category);
//             if (searchParams.state !== 'all') queryParams.append('state_code', searchParams.state);
//             if (searchParams.country !== 'all') queryParams.append('country_code', searchParams.country);
...
//     const handleSelectChange = (value: string, field: 'category' | 'state' | 'country') => {
//         setSearchParams(prev => ({ ...prev, [field]: value }));
//     };
