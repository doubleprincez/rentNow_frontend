...
import {BanknoteIcon, CheckCheckIcon} from "lucide-react";
    ...
    const gateways: GatewayI[] = [
        {
            title: 'Bank Transfer',
            'slug': 'bank-transfer',
            component: (<BankTransfer plan={selectedPlan} onGoBack={removePlan} onCompleted={setGatewayResponse}/>),
            active: true,
            icon: <BanknoteIcon/>,
            className: "text-lg font-bold bg-gray-800 text-white rounded hover:shadow-lg"
        },
        {
            title: 'Paystack',
            'slug': 'paystack',
    ...
    const PlanSelection = () => {
        return <div>
            ...
            <div className="grid md:grid-cols-2"> {
                ...
                plans?.map((pl: PlansInterface, index: number) => <div key={index} className="relative text-gray-300"
                                                                       id="pricing">
                            ...
                            <div
                                ...
                                <div>
                                    <ul className="space-y-4 mb-4">
                                        {
                                            pl?.features && pl.features.map((ft: FeatureInterface, i: number) => <li
                                                key={i} className="space-y-2 w-full">
                                                <div className="flex "><CheckCheckIcon/><label
                                                    className="text-bold text-white "> {" " + ft?.name}</label></div>
                                ...
                                </div>
                                {
                                    ...
                                    ><span className="relative text-sm font-semibold text-black" onClick={() => {
                                        pl.id && selectedPlanIs(pl.id)
        ...
        return <>
            <div className="py-6 ">
                ...
                <div className="p-4 space-y-3">
                    ...
                    <p>
                        <strong>@ {selectedPlan?.currency}{formatAmountNumber(selectedPlan?.price)} </strong> / {selectedPlan?.invoice_interval}
                    </p>
                ...
                <div className="p-3 mt-20">
                    <div className={"flex flex-wrap space-x-2 md:space-x-8  justify-start md:justify-between"}>
                        ...
                        {
                            gateways.map((way: any, i: number) => <div key={i}>
                                <button className={" px-2 py-2 md:py-4 md:px-3  cursor-pointer " + way.className}
                ...
                <div className="pt-20">
                    <Button className="rounded border border-red-300 hover:bg-red-200"
                            onClick={() => removePlan()}>Back</Button>
                </div>
            </div>
        </>
    }
